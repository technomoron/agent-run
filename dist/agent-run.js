...
function loadManifest(_configRoot, agentDir, profile) {
    const manifestPath = path.join(agentDir, MANIFEST_FILE_NAME);
    if (!fs.existsSync(manifestPath)) {
...
function buildRenderContext(projectRoot, agentDir, configRoot, manifest, env) {
...
function renderTemplateFile(env, configRoot, templatePath, context) {
...
function renderSkills(env, configRoot, manifest, context) {
    const skills = [];
...
function renderNativeSkillFiles(context) {
    const files = [];
...
function renderToolInstructions(templateName, env, configRoot, context, isClaude) {
...
function renderCodexConfig(env, configRoot, context) {
...
function renderClaudeSettings(env, configRoot, context) {
...
function renderLegacyAgentsMods(sourceFile, stack = []) {
...
function runCodex(realBinary, permissionArgs, agentDir, args, projectRoot, wrapperArgs) {
    ...
    execCommand(realBinary, [
        ...
        ...(wrapperArgs.codexSandboxMode === 'sandboxed' && wrapperArgs.codexNetwork
            ...
            : []),
        ...args
    ], shouldUseShell(realBinary), env, codexHomeDir, (code) => postflightProjectCheck(projectRoot, agentDir, code));
}
