...
const builtInSkills = {
    ...
    'todo-manager': {
        ...
        content: 'Use todo_list and todo_get when asked to inspect saved tasks. Create tasks only when the user asks to track them or has explicitly authorized a task-tracking workflow. A request to do work does not by itself authorize creating a saved task. Update using the current revision to avoid overwriting concurrent changes. Distinguish todo, doing, blocked, done, and cancelled. Imported tasks retain their external source identity. Local changes do not imply permission to update remote systems.'
    }
};
