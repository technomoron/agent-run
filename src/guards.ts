...
function posixGitShim(): string {
	return [
		...
		'  case "$arg" in',
		'    -C|-c|--git-dir|--work-tree|--namespace|--super-prefix|--config-env) skip_value=1; continue ;;',
		'    --help|-h|--version|-v|--html-path|--man-path|--info-path|--) break ;;',
		'    -*) continue ;;',
...
function windowsShim(name: string): string {
	...
	return [
		...
		'set "AGENT_RUN_GUARD_SELF=%~f0"',
		'for %%D in ("%~dp0.") do set "AGENT_RUN_GUARD_DIR=%%~fD"',
		...guard,
		':run',
		'if defined AGENT_RUN_REAL_PATH (',
		'  set "PATH=%AGENT_RUN_REAL_PATH%"',
		') else (',
		'  for /f "tokens=1,* delims=;" %%A in ("%PATH%") do for %%D in ("%%~A\\.") do if /I "%%~fD"=="%AGENT_RUN_GUARD_DIR%" set "PATH=%%B"',
		')',
		'set "AGENT_RUN_COMMAND="',
		`for %%I in (${executable}) do set "AGENT_RUN_COMMAND=%%~$PATH:I"`,
		'if not defined AGENT_RUN_COMMAND (',
		`  echo agent-run: cannot find the real ${name} command outside the guard directory. 1>&2`,
		'  exit /b 127',
		')',
		'if /I "%AGENT_RUN_COMMAND%"=="%AGENT_RUN_GUARD_SELF%" (',
		`  echo agent-run: refused recursive ${name} guard resolution. 1>&2`,
		'  exit /b 127',
		')',
		'"%AGENT_RUN_COMMAND%" %*',
		'exit /b %ERRORLEVEL%',
...
function windowsGitGuard(): string[] {
	return [
		':scan',
		'if "%~1"=="" goto run',
		'if "%~1"=="-C" goto skip_git_value',
		'if "%~1"=="-c" goto skip_git_value',
		'if "%~1"=="--git-dir" goto skip_git_value',
		'if "%~1"=="--work-tree" goto skip_git_value',
		'if "%~1"=="--namespace" goto skip_git_value',
		'if "%~1"=="--super-prefix" goto skip_git_value',
		'if "%~1"=="--config-env" goto skip_git_value',
		'if "%~1"=="--help" goto run',
		'if "%~1"=="-h" goto run',
		'if "%~1"=="--version" goto run',
		'if "%~1"=="-v" goto run',
		'if "%~1"=="--html-path" goto run',
		'if "%~1"=="--man-path" goto run',
		'if "%~1"=="--info-path" goto run',
		'if "%~1"=="--" goto run',
		'if /I "%~1"=="commit" goto block_git',
		'if /I "%~1"=="tag" goto block_git',
		'if /I "%~1"=="push" goto block_git',
		'set "ARG=%~1"',
		'if not "%ARG:~0,1%"=="-" goto run',
		'shift',
		'goto scan',
		':skip_git_value',
		'shift',
...
function windowsGitTestRepo(): string[] {
	return [
		...
		'if "%~1"=="-C" goto agent_run_chdir',
		'if "%~1"=="-c" (shift & shift & goto agent_run_scan)',
		'set "ARG=%~1"',
...
function windowsGhGuard(): string[] {
	return [
		'set "SAW_RELEASE=0"',
		':scan',
		'if "%~1"=="" goto run',
...
function windowsPublishGuard(name: string): string[] {
	return [
		':scan',
		'if "%~1"=="" goto run',
